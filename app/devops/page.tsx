...
import { Server, Briefcase, Cpu, ShoppingBag, Cloud, Database, Workflow, FileCode, ShoppingCart } from "lucide-react";
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {devopsPackages.map((pkg, i) => {
                            ...
                            return (
                                <div
                                    key={i}
                                    className="group relative p-6 rounded-3xl border bg-white/[0.02] border-white/10 hover:border-purple-500/50 hover:bg-white/[0.04] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_10px_30px_-10px_rgba(168,85,247,0.15)] flex flex-col justify-between cursor-pointer"
                                >
                                    <div>
                                        <div className="flex justify-between items-center mb-3">
                                            <span className="text-[10px] font-mono text-purple-400 font-bold group-hover:tracking-wider transition-all duration-300">{pkg.cat}</span>
                                            <div className="p-2 rounded-xl bg-purple-500/10 group-hover:bg-purple-500/20 group-hover:scale-110 transition-all duration-300">
                                                <IconComponent className="w-4 h-4 text-purple-300" />
                                            </div>
                                        </div>
                                        <h3 className="text-lg font-bold text-white group-hover:text-purple-300 transition-colors duration-300">{pkg.title}</h3>
                                        <p className="text-xs text-zinc-400 mt-1 mb-4 leading-relaxed">{pkg.desc}</p>
                                    </div>
                                    <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                        <span className="text-xl font-bold font-mono text-white group-hover:scale-105 transition-transform duration-300">από {pkg.price}€</span>
                                        <button
                                            onClick={() => handleAddToCart(pkg)}
                                            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs transition-all duration-300 shadow hover:shadow-lg hover:shadow-purple-500/25 active:scale-95 cursor-pointer"
                                        >
