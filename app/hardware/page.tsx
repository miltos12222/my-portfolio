...
export default function HardwarePage() {
    ...
    // Dynamic accent color based on active category
    const accentGlow = buildCategory === "gaming" ? "hover:border-emerald-500/50 hover:shadow-[0_15px_40px_-15px_rgba(16,185,129,0.2)]" : buildCategory === "workstation" ? "hover:border-cyan-500/50 hover:shadow-[0_15px_40px_-15px_rgba(6,182,212,0.2)]" : "hover:border-purple-500/50 hover:shadow-[0_15px_40px_-15px_rgba(168,85,247,0.2)]";
                ...
                <div className="text-center max-w-2xl mx-auto space-y-4 reveal-on-scroll">
                    ...
                    <div className="flex justify-center gap-3 pt-2">
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-2 hover:scale-105 active:scale-95 ${mode === "builder" ? "bg-emerald-500 text-black shadow-lg shadow-emerald-500/20" : "bg-white/[0.05] border border-white/10 text-zinc-300 hover:text-white"}`}
                        >
                        ...
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-2 hover:scale-105 active:scale-95 ${mode === "repair" ? "bg-cyan-500 text-black shadow-lg shadow-cyan-500/20" : "bg-white/[0.05] border border-white/10 text-zinc-300 hover:text-white"}`}
                        >
                ...
                {mode === "builder" && (
                    <div className="space-y-8 animate-fade-in max-w-3xl mx-auto reveal-on-scroll">
                        ...
                        <div className="flex justify-center items-center gap-2 sm:gap-4 text-xs font-mono">
                            <button onClick={() => setStep(1)} className={`px-4 py-2 rounded-xl border cursor-pointer transition-all ${step === 1 ? "bg-emerald-500 text-black font-bold border-emerald-500 shadow-md" : "bg-white/[0.03] border-white/10 text-zinc-400 hover:text-white"}`}>
                                1. Διαμόρφωση Υλικών
                            </button>
                            <button onClick={() => setStep(2)} className={`px-4 py-2 rounded-xl border cursor-pointer transition-all ${step === 2 ? "bg-emerald-500 text-black font-bold border-emerald-500 shadow-md" : "bg-white/[0.03] border-white/10 text-zinc-400 hover:text-white"}`}>
                                2. Σύνοψη & Αίτηση
                            </button>
                        </div>
                        ...
                        {step === 1 ? (
                            <div className="space-y-8">
                                {/* Category Selector with Smooth Hover Animations */}
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    {[
                                        ...
                                        <div
                                            ...
                                            className={`group p-5 rounded-2xl border transition-all duration-300 cursor-pointer flex flex-col justify-between hover:-translate-y-1 hover:scale-[1.02] ${buildCategory === cat.id ? "bg-emerald-500/15 border-emerald-500 shadow-lg shadow-emerald-500/10" : "bg-white/[0.02] border-white/10 hover:border-emerald-500/30 hover:bg-white/[0.04]"}`}
                                        >
                                            <div className="space-y-1">
                                                <h3 className="text-sm font-bold text-white group-hover:text-emerald-300 transition-colors">{cat.title}</h3>
                                                <p className="text-[11px] text-zinc-400 leading-relaxed">{cat.desc}</p>
                                            </div>
                                            {buildCategory === cat.id && <span className="mt-3 text-[10px] font-mono text-emerald-400 font-bold flex items-center gap-1 animate-pulse"><Check className="w-3 h-3" /> Επιλεγμένο</span>}
                                        </div>
                                    ))}
                                </div>

                                {/* Dropdown Options Box with Dynamic Glow */}
                                <div className={`bg-white/[0.02] border border-white/10 p-6 sm:p-8 rounded-3xl space-y-6 transition-all duration-500 ${accentGlow}`}>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <select
                                                ...
                                                className="w-full px-4 py-3 rounded-xl bg-white/[0.05] border border-white/10 text-xs font-mono text-white focus:outline-none focus:border-emerald-500 cursor-pointer transition-colors"
                                            >
                                    ...
                                    <div className="pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 bg-emerald-500/5 p-4 rounded-2xl border border-emerald-500/20">
                                        ...
                                        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                                            <button
                                                ...
                                                className="flex-1 sm:flex-none px-5 py-3 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-bold text-xs transition-all duration-300 shadow hover:shadow-lg hover:shadow-cyan-500/25 active:scale-95 cursor-pointer flex items-center justify-center gap-2"
                                            >
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setStep(2)}
                                                className="flex-1 sm:flex-none px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-bold text-xs transition-all duration-300 shadow hover:shadow-lg hover:shadow-emerald-500/25 active:scale-95 cursor-pointer flex items-center justify-center gap-2"
                                            >
                            ...
                            <div className="space-y-6 max-w-2xl mx-auto">
                                <div className="bg-white/[0.03] border border-emerald-500/30 p-6 rounded-3xl space-y-4 shadow-xl">
                                    ...
                                    <form onSubmit={handleSubmitOrder} className="space-y-5">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                            <div>
                                                ...
                                                <input type="text" required placeholder="Γιάννης Παπαδόπουλος" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-500 transition-colors" />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-medium text-zinc-300 mb-2">Email</label>
                                                <input type="email" required placeholder="example@domain.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-500 transition-colors" />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium opacity-80 mb-2">Παρατηρήσεις</label>
                                            <textarea rows={3} placeholder="Προτιμήσεις..." value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/15 text-sm focus:outline-none focus:border-emerald-500 transition-colors resize-none" />
                                        </div>
                                        <div className="flex gap-3">
                                            ...
                                            <button type="submit" disabled={isSubmitting} className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-black font-bold text-sm shadow cursor-pointer hover:scale-[1.01] active:scale-95 transition-all">
                                        ...
                                        <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-bold text-xs transition-all duration-300 shadow hover:shadow-lg hover:shadow-cyan-500/25 active:scale-95 cursor-pointer"
                                                >
                                                    <ShoppingCart className="w-3.5 h-3.5" />
                                                    <span>Καλάθι</span>
                                                </button>
                                                <span onClick={() => setSelectedRepair(`${rep.title} (${rep.price}€)`)} className={`text-xs px-3 py-2 rounded-xl font-medium transition-all cursor-pointer ${isSelected ? "bg-cyan-500 text-black font-bold" : "bg-white/10 text-white hover:bg-white/20"}`}>
